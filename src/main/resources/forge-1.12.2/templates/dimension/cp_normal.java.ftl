public static class ${name}ChunkProvider implements IChunkGenerator {
	...
	public ${name}ChunkProvider(World worldIn, long seed) {
		...
		ravineGenerator = new MapGenRavine() {
			@Override protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
				...
				if (state.getBlock() == STONE.getBlock() || state.getBlock() == biome.topBlock.getBlock() || state.getBlock() == biome.fillerBlock.getBlock()) {
					if (y - 1 < 10) {
	...
	@Override public void populate(int x, int z) {
		...
		if (this.random.nextInt(4) == 0)
			if (TerrainGen.populate(this, this.world, this.random, x, z, false, PopulateChunkEvent.Populate.EventType.LAKE)) {
	...
	@Override public BlockPos getNearestStructurePos(World worldIn, String structureName, BlockPos position, boolean findUnexplored) {
		return null;
	}
